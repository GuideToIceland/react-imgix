...
function buildDprSrcFunction(url, disableQualityByDPR, quality) {
  ...
  return function (dpr) {
    return "".concat(url, "&q=").concat(quality || CONSTANTS["q_dpr".concat(dpr)], "&dpr=").concat(dpr, " ").concat(dpr, "x");
  };
}
/**
...
function buildSrc(_ref) {
  ...
  if (disableSrcSet) {
    ...
    if (fixedSize || type === "source") {
      ...
      var constructedUrl = constructUrl(srcWithoutQ, urlParams);
      ...
      var _constructedUrl = constructUrl(rawSrc, _urlParams);
